...
export const PromptBox = React.forwardRef<HTMLTextAreaElement, React.TextareaHTMLAttributes<HTMLTextAreaElement>>(
  ({ className, ...props }, ref) => {
    ...
    React.useLayoutEffect(() => { 
      ...
      } else if (textarea && props.readOnly) {
        // For read-only, keep it at fixed single-line height
    ...
    return (
      <div className={cn("flex flex-col rounded-[28px] p-2 shadow-sm transition-colors bg-white border dark:bg-[#303030] dark:border-transparent cursor-text", className)}>
        ...
        <div className="w-full p-3 text-foreground dark:text-white min-h-12 flex items-center cursor-pointer" onClick={(e) => { if (props.readOnly) { const form = e.currentTarget.closest('form'); if (form) { form.requestSubmit(); } } }}>
          <span className={cn(props.readOnly ? "select-none" : "")}>{props.value || value || props.placeholder}</span>
        </div>
        <textarea ref={internalTextareaRef} rows={1} value={value} onChange={handleInputChange} placeholder="Message..." className="hidden w-full resize-none border-0 bg-transparent p-3 text-foreground dark:text-white placeholder:text-muted-foreground dark:placeholder:text-gray-300 focus:ring-0 focus-visible:outline-none min-h-12 overflow-hidden" {...props} />
          ...
          <TooltipProvider delayDuration={100}>
            <div className="flex items-center gap-2">
              ...
              <div className="ml-auto flex items-center gap-2">
                ...
                <Tooltip>
                  <TooltipTrigger asChild>
                    <button type="submit" disabled={!hasValue} className={cn(
                      "flex h-8 w-8 items-center justify-center rounded-full text-sm font-medium transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring disabled:pointer-events-none",
                      hasValue 
                        ? "bg-gradient-to-br from-sky-500 to-blue-600 text-white hover:from-sky-600 hover:to-blue-700 shadow-lg shadow-sky-500/30" 
                        : "bg-black/40 dark:bg-[#515151] text-white"
                    )}>
