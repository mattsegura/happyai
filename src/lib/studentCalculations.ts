...
export function calculateMoodVariability(moodHistory: SentimentHistory[]): {
  ...
  const variance =
    squaredDiffs.reduce((sum, val) => sum + val, 0) / intensities.length;
  ...
  const firstAvg =
    firstHalf.reduce((sum, m) => sum + m.intensity, 0) / firstHalf.length;
  const secondAvg =
    secondHalf.reduce((sum, m) => sum + m.intensity, 0) / secondHalf.length;
...
export function calculateGradeTrend(assignments: AssignmentWithStatus[]): {
  ...
  const recentAvg =
    recent.reduce((sum, a) => sum + (a.score! / a.points_possible) * 100, 0) /
    recent.length;
  const previousAvg =
    previous.length > 0
      ? previous.reduce(
          (sum, a) => sum + (a.score! / a.points_possible) * 100,
          0
        ) / previous.length
      : recentAvg;
...
export function calculateOverallParticipation(
  ...
  const totalPulses = participationData.reduce(
    (sum, p) => sum + p.total_pulses,
    0
  );
  const completedPulses = participationData.reduce(
    (sum, p) => sum + p.completed_pulses,
    0
  );
  const averageRate =
    totalPulses > 0 ? Math.round((completedPulses / totalPulses) * 100) : 0;
  const totalPoints = participationData.reduce(
  ...
  const avgRank = Math.round(
    participationData.reduce((sum, p) => sum + p.rank, 0) /
      participationData.length
  );
  const totalStudents = Math.max(
    ...participationData.map((p) => p.total_students)
  );
...
export function generateEmotionalTrajectory(moodHistory: SentimentHistory[]): {
  ...
  if (moodHistory.length === 0) {
    return {
      summary:
        "Start tracking your mood to see insights about your emotional wellbeing.",
      dominantEmotions: [],
  ...
  const avgIntensity =
    moodHistory.reduce((sum, m) => sum + m.intensity, 0) / moodHistory.length;
  ...
  if (trend === "improving" && avgIntensity >= 5) {
    summary = `Your emotional wellbeing has been **steadily improving** over the past ${
      moodHistory.length
    } days. You're experiencing more positive emotions (${dominantEmotions
      .slice(0, 2)
      .join(
        ", "
      )}) and maintaining good emotional balance. Keep up the great work! 🌟`;
    recommendation =
      "Continue your current habits and routines that are supporting your wellbeing.";
    ...
    recommendation =
      "Keep engaging with support resources and maintain your self-care practices.";
  } else if (trend === "declining" && avgIntensity < 4) {
    summary = `Your emotional wellbeing has been **declining** recently, with more frequent feelings of ${
      dominantEmotions[0]?.toLowerCase() || "stress"
    }. This is a signal that you may need additional support. Remember, it's okay to ask for help. 🤗`;
    recommendation =
      "Consider reaching out to a counselor, talking to a trusted friend, or using campus wellness resources.";
  } else if (trend === "declining") {
    summary = `Your mood has been **slightly declining** over the past ${
      moodHistory.length
    } days. You're experiencing more ${
      dominantEmotions[0]?.toLowerCase() || "challenging"
    } emotions lately. Let's work on getting you back on track. 💙`;
    recommendation =
      "Try scheduling some self-care time, connecting with friends, or using stress-reduction techniques.";
  } else {
    summary = `Your emotional wellbeing has been **stable** over the past ${
      moodHistory.length
    } days, with consistent feelings of ${
      dominantEmotions[0]?.toLowerCase() || "balance"
    }. You're maintaining a steady emotional state. 😊`;
    recommendation =
      "Continue monitoring your mood and maintain your current balance.";
  }
...
export function calculateBadgeProgress(
  ...
  switch (badgeId) {
    ...
    case "badge-2": // Consistent Performer (5 assignments 90%+)
      const highScores = assignments.filter(
        (a) => a.score && a.score / a.points_possible >= 0.9
      ).length;
      return Math.min(100, (highScores / 5) * 100);

    case "badge-4": // Early Bird (5 early submissions)
      const earlySubmissions = assignments.filter(
        (a) =>
          a.submitted_at &&
          a.due_at &&
          new Date(a.submitted_at) < new Date(a.due_at)
      ).length;
    ...
    case "badge-11": // Positive Vibes (7 days positive)
      const recentPositive = moodHistory
        .slice(-7)
        .filter((m) => m.intensity >= 5).length;
